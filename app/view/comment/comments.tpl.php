...
<?php if (is_array($comments)) : ?>
    ...
    <?php foreach ($comments as $id => $comment) : ?>
            <div class='comment-container'>
                ...
                <div class='comment-id'><a href="<?=$this->url->create('comment/edit-view/' . $id . '/' . $pagekey)?>">#<?=$id?></a></div>
                <div class='comment-text'><?=$comment['content']?></div>
                
                <div class='comment-author-container'>
                    <div class='comment-time'><?=date("j F Y k\l.G:i", $comment['timestamp'])?></div>
                    <div class='author-name'><?=$comment['name']?></div>
                    <div class='author-contact'>
                        <a href='mailto:<?=$comment['mail']?>'>E-mail</a> | 
